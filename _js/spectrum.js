var zx = {
    ...
    vram: new Array(), // Array da 0 a 767 (pari a una matrice 32 x 24) che contiene per ogni byte dello schermo
                       // il carattere contenuto, il colore di sfondo ed il colore di primo piano

    refresh_border: false,

    refresh_screen_end: -1,

    border_color: "",

    palette: new Array("#000000", "#0000c0", "#c00000", "#c000c0",  // Palette approssimativa dei colori
                       "#00c000", "#00c0c0", "#c0c000", "#c0c0c0",  // disponibili su di un vero ZX Spectrum
                       "#000000", "#0000ff", "#ff0000", "#ff00ff",
                       "#00ff00", "#00ffff", "#ffff00", "#ffffff"),

    mainCounter: 0, // Contatore

    loopStillWorking: false,
    ...
    setPixel: function(P_nCol, P_nRow, P_cChar, P_nColor, P_nBackground) {
        ...
        zx.vram[nPos]["foreground"] = zx.palette[P_nColor];
        zx.vram[nPos]["background"] = zx.palette[P_nBackground];
    ...
    setBorder: function(P_nColore) {
        if (P_nColore >= 0) {
            zx.border_color = zx.palette[P_nColore];
        } else {
            zx.border_color = zx.palette[5] + " url(_img/background_loading.gif) repeat left top";
        }
        zx.refresh_border = true;
    },
    ...
    setText: function(P_nCol, P_nRow, P_cText, P_nColor, P_nBackground) {
        ...
        while (nInd < acChars.length) {
            zx.setPixel(P_nCol + nInd, P_nRow, acChars[nInd], P_nColor, P_nBackground);
            nInd += 1;
        }
    },
...
function inizializza() {
    ...
    while (nRiga < 24) {
        ...
        while (nColonna < 32) {
            ...
            zx.vram.push({"char": "&nbsp;", "foreground": "#000", "background": "#fff"});
...
function setLoopPrincipale() {
    var timerLoop = setInterval(function() {
        if (!zx.loopStillWorking) {
            ...
            refreshSchermo();
            if (zx.refresh_border) {
                $("html").css("background", zx.border_color);
                zx.refresh_border = false;
            }
            zx.mainCounter += 1;
...
$.simulaInizioCaricamento = function(P_nRitardo, P_cNomeFile) {
    zx.wait(P_nRitardo, "zx.setBorder(5)");
    zx.wait(P_nRitardo + 600, "zx.setBorder(2)");
    zx.wait(P_nRitardo + 1200, "zx.setBorder(5)");
    zx.wait(P_nRitardo + 1400, "zx.setBorder(2)");
    zx.wait(P_nRitardo + 1600, "zx.setBorder(-1)");
    zx.wait(P_nRitardo + 2400, "zx.setBorder(2)");
    zx.wait(P_nRitardo + 2400, "zx.setText(0, 1, 'Program: " + P_cNomeFile + "');");
    zx.wait(P_nRitardo + 3000, "zx.setBorder(5)");
    zx.wait(P_nRitardo + 3400, "zx.setBorder(2)");
    zx.wait(P_nRitardo + 4000, "zx.setBorder(5)");
    zx.wait(P_nRitardo + 4100, "zx.setBorder(2)");
    zx.wait(P_nRitardo + 4200, "zx.setBorder(-1)");
};

$.simulaTermineCaricamento = function(P_nRitardo, P_lCaricamentoABuonFine) {
    zx.wait(P_nRitardo, "zx.setBorder(2)");
    if (P_lCaricamentoABuonFine) {
        zx.wait(P_nRitardo + 100, "zx.setBorder(0)");
        zx.wait(P_nRitardo + 300, "zx.setBorder(15)");
        zx.wait(P_nRitardo + 300, "zx.clearScreen()");
    } else {
        zx.wait(P_nRitardo + 100, "zx.setBorder(15)");
